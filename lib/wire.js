...
//   https://anakin.io/wire → pick "Flipkart" or "YouTube" catalog
//                          → click an action → copy its action_id
// Then set the corresponding env vars in .env.local
// ---------------------------------------------------------------------------

const ACTION_FLIPKART_SEARCH = process.env.WIRE_ACTION_FLIPKART_SEARCH;
const ACTION_FLIPKART_DETAILS = process.env.WIRE_ACTION_FLIPKART_DETAILS;
const ACTION_FLIPKART_REVIEWS = process.env.WIRE_ACTION_FLIPKART_REVIEWS;
...
/**
 * Search Flipkart for products matching a query.
 *
 ...
 */
export async function searchFlipkartProducts(query) {
  const result = await wireCall(requireAction(ACTION_FLIPKART_SEARCH, "FLIPKART_SEARCH"), {
    query,
    limit: 24,
    sort: "featured",
  });
  ...
  const arr =
    ...
    [];
  console.log(`[wire] searchFlipkartProducts → ${arr.length} items`);
  return arr;
}
...
export async function getProductDetails(asin) {
  const result = await wireCall(requireAction(ACTION_FLIPKART_DETAILS, "FLIPKART_DETAILS"), { asin });
  if (!result) return {};
...
/**
 * Fetch Flipkart reviews for a product.
 *
...
export async function getProductReviews(asin) {
  const result = await wireCall(requireAction(ACTION_FLIPKART_REVIEWS, "FLIPKART_REVIEWS"), { asin });
  if (!result) return [];
  const arr =
    (Array.isArray(result) ? result : null) ||
